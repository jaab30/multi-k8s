...
docker build -t jaab30/multi-server:latest -f jaab30/multi-server:$SHA ./server/Dockerfile ./server
docker build -t jaab30/multi-worker:latest -f jaab30/multi-worker:$SHA ./worker/Dockerfile ./worker
...
kubectl set image deployments/client-deployment client=jaab30/multi-server:$SHA
kubectl set image deployments/server-deployment server=jaab30/multi-server:$SHA